...
from ctxcore.rnkdb import *
from pyCistrome.cluster_buster import *
import random
from scipy.stats import ranksums
import scipy.sparse as sparse
import ray
...
class DEM():
    ...
    def run(self, dem_db_scores, **kwargs):
        ...
        motif_enrichment = add_motif_annotation(motif_enrichment,
                                               specie = self.specie,
                                               version = self.annotation_version,
                                               path_to_motif_annotations = self.path_to_motif_annotations)
        self.motif_enrichment = motif_enrichment
        # Format motif hits
...
def DEM_ray(dem_db_scores: pd.DataFrame,
    ...
    motif_df['Number_of_regions'] = [len(motif_hits_list[i][0]) for i in range(len(keep_motifs))]
    motif_df['Number_of_regions'] = motif_df['Number_of_regions'].astype(int)
...
def add_motif_annotation(motif_enrichment_dict: Dict[str, pd.DataFrame],
    ...
    try:
        ...
        motif_enrichment_dict_w_annot = {key: pd.concat([motif_enrichment_dict[key], annot_df], axis=1, sort=False).loc[motif_enrichment_dict[key].index.tolist(),:] for key in motif_enrichment_dict.keys()}
    except:
        log.info('Unable to load annotation for ' + specie)
        annot_df = None
        motif_enrichment_dict_w_annot = motif_enrichment_dict
    # Add info to elements in dict
    if add_logo == True:
        for key in motif_enrichment_dict.keys():
            motif_enrichment_dict_w_annot[key]['Logo']=['<img src="' +'https://motifcollections.aertslab.org/' + version + '/logos/'+ motif_enrichment_dict_w_annot[key].index.tolist()[i] + '.png' + '" width="200" >' for i in range(motif_enrichment_dict_w_annot[key].shape[0])]
        if annot_df is not None:
            ...
            motif_enrichment_dict_w_annot = {key: motif_enrichment_dict_w_annot[key].loc[:,['Logo', 'Contrast', 'Log2FC', 'Adjusted_pval', 'Mean_fg', 'Mean_bg', 'Motif_hit_thr', 'Number_of_regions']] for key in motif_enrichment_dict_w_annot.keys()}
    else:
        if annot_df is not None:
            motif_enrichment_dict_w_annot = {key: motif_enrichment_dict_w_annot[key].loc[:,['Contrast', 'Direct_annot', 'Motif_similarity_annot', 'Orthology_annot', 'Motif_similarity_and_Orthology_annot', 'Log2FC', 'Adjusted_pval', 'Mean_fg', 'Mean_bg', 'Motif_hit_thr', 'Number_of_regions']] for key in motif_enrichment_dict_w_annot.keys()}
        else:
            motif_enrichment_dict_w_annot = {key: motif_enrichment_dict_w_annot[key].loc[:,['Contrast', 'Log2FC', 'Adjusted_pval', 'Mean_fg', 'Mean_bg', 'Motif_hit_thr', 'Number_of_regions']] for key in motif_enrichment_dict_w_annot.keys()}
        
    return motif_enrichment_dict_w_annot 
